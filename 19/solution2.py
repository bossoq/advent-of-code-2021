...
def manhattan_distance(pos_lst):
    dists = list()
    for pos1 in pos_lst:
        for pos2 in pos_lst:
            dists.append(abs(pos1[0] - pos2[0]) + abs(pos1[1] - pos2[1]) +
                         abs(pos1[2] - pos2[2]))
    return dists


beacons, scanners = create_maps(all_input)
dists_list = manhattan_distance(scanners)
print(f'Max Manhattan distance: {max(dists_list)}')
